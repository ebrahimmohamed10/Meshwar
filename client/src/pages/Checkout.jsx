...
const Checkout = () => {
  const { id } = useParams()
  const { cars, axios, pickupDate, returnDate, currency, user } = useAppContext()
    ...
    <div className='min-h-screen bg-gray-50 flex flex-col'>
      ...
      <div className='flex-grow px-4 md:px-16 lg:px-24 xl:px-32 py-10'>
        <div className='max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10'>
          ...
          >
            {/* Wallet Checkout */}
            {user && user.wallet >= grandTotal && (
              <div className='bg-white/70 backdrop-blur-xl border border-white/40 p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] mb-8'>
                <h2 className='text-lg font-medium text-gray-800 mb-4 flex items-center gap-2'>
                   Wallet Payment
                </h2>
                <div className='flex flex-col gap-4'>
                   <button onClick={(e) => handlePayment(e, 'Wallet')} disabled={loading || success} className={`w-full py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm ${success ? 'bg-green-500 text-white' : 'bg-primary hover:bg-primary-dull text-white cursor-pointer hover:scale-[1.02] disabled:opacity-70 disabled:hover:scale-100'}`}>
                     {loading ? (
                        ...
                        <>
                           <svg className="lucide lucide-wallet w-5 h-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 7V4a1 1 0 0 0-1-1H5a2 2 0 0 0 0 4h15a1 1 0 0 1 1 1v4h-3a2 2 0 0 0 0 4h3a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1" /><path d="M3 5v14a2 2 0 0 0 2 2h15a1 1 0 0 0 1-1v-4" /></svg>
                           <span className="font-semibold text-[17px] tracking-wide">Pay with Wallet (Balance: {user.wallet?.toLocaleString()} {currency})</span>
                        </>
                     )}
                   </button>
                </div>
            ...
            <div className='bg-white/70 backdrop-blur-xl border border-white/40 p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)]'>
               ...
               <div className='flex flex-col gap-4'>
                  <button onClick={(e) => handlePayment(e, 'Apple Pay')} disabled={loading || success} className={`w-full py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-transform shadow-sm ${success ? 'bg-green-500 text-white' : 'bg-[#000] hover:bg-gray-800 text-white cursor-pointer hover:scale-[1.02] disabled:opacity-70 disabled:hover:scale-100'}`}>
                    {loading ? (
                       <span className='flex items-center gap-2'>
                          <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                             <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                       ...
                       <>
                          <svg className="w-5 h-5 mb-0.5" viewBox="0 0 384 512" fill="currentColor"><path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-90-61.7-91.9zm-56.6-164.2c27.3-32.4 24.8-61.9 24-72.5-24.1 1.4-52 16.4-67.9 34.9-17.5 19.8-27.8 44.3-25.6 71.9 26.1 2 49.9-11.4 69.5-34.3z"/></svg>
                          <span className="font-semibold text-[17px] tracking-wide">Pay</span>
                       </>
                    )}
                  </button>
               </div>
            ...
            {/* Manual Form */}
            <form onSubmit={(e) => handlePayment(e, 'Credit Card')} className='bg-white/70 backdrop-blur-xl border border-white/40 p-6 md:p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-6'>
